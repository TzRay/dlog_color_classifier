...
开启伴随文件整理后，视频与同名伴随文件会使用一致的编号。选择“跳过冲突”或
“标记为失败”时，只要其中一个目标名称冲突，就会保留整组原文件。

文件明细每页显示最多 100 条，可以翻页或搜索；筛选与搜索覆盖全部扫描结果。
扫描和整理期间会显示进度及“取消任务”按钮。复制中途取消时，未完成的临时副本
会被清理，已经完成的文件仍会保留并计入结果；再次整理前需要重新识别。
